...
int main(int argc, char* argv[]) {
  ...
  if (setAffRet != 0) {
    std::cout
        << "setting up CPU-affinity failed. teminating."
        << "\n";
    std::terminate();
  }
...
for(int i = 0; i < nMsgs; ++i){
	...
	latency = static_cast<double>(duration_cast<std::chrono::nanoseconds>(completionTime - startTime).count());
	varIndex = readRet.value().index();
	latencies[i] = latency;
}
...
//write vector of all latencies to csv
{
