...
void fileToTuplesLogic(const std::string& filePath,
  ...
  while (std::getline(inputStream, line)) {
    ...
    (([&]() {
       ...
       // template stoArith to convert string to respective type
       try {
         std::get<Is>(tempTuple) =
             Auxil::stoArith<std::tuple_element_t<Is, LineTuple>>(buffer);
       } catch (const std::exception& ex) {
         std::get<Is>(tempTuple) = 0;
       }
     })(),
...
requires Auxil::validate_tuple<LineTuple, std::is_arithmetic>::value
    std::vector<LineTuple>
    fileToTuples(const std::string& filePath, char delimiter = ',') {
